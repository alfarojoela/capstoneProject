...
void Player::usePowerUp(std::string powerUp)
{
	if (checkInventory(powerUp) == 1)
	{
		std::cout << "You use the " << powerUp << std::endl;
	}
	else
		std::cout << "You don't have " <<powerUp << " in your inventory." << std::endl;
}
...
void Player::setGrit(Item powerup)
	...
	void Player::setAlive(bool death)
	{
		if (death == 0)
		{
			std::cout << '\a';
			std::cout << "Gosh darn it!  You died!" << std::endl;
